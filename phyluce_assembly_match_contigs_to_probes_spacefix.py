...
def main():
    ...
    for contig in sorted(fasta_files):
        ...
        # Build space-safe paths for lastz (quote if they contain spaces)
        contig_for_lastz = contig
        probes_for_lastz = args.probes
        output_for_lastz = output

        if " " in contig_for_lastz:
            contig_for_lastz = '"{}"'.format(contig_for_lastz)
        if " " in probes_for_lastz:
            probes_for_lastz = '"{}"'.format(probes_for_lastz)
        if " " in output_for_lastz:
            output_for_lastz = '"{}"'.format(output_for_lastz)
